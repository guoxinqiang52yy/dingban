<!--
 * @Author: 登录页面
 * @Date: 2020-08-12 10:05:26
 * @LastEditTime: 2020-12-22 18:44:05
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \backstage\src\pages\login.vue
-->
  ...
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="demo-ruleForm login-container">
      ...
      <el-form-item prop="username">
        <el-input type="text" v-model="ruleForm.username" auto-complete="off" placeholder="手机号"> </el-input>
      </el-form-item>
      <el-form-item prop="verify" class="verify">
        <el-input v-model="ruleForm.verify" auto-complete="off" placeholder="验证码"></el-input>
        <el-button type="primary" :disabled="disabled" @click="getYangZheng">{{ yangName }}</el-button>
      </el-form-item>
      <el-form-item style="width:100%;">
        <el-button type="primary" style="width:100%;" @click="submitForm('ruleForm')" :loading="logining">登录</el-button>
        <!-- <el-button type="primary" style="width:100%;" @click="gotoPage2" :loading="logining">登录no</el-button> -->
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getYangZheng() {
      //判断手机号
      this.$refs.ruleForm.validateField("username", (val) => {
        if (val) {
          this.showMessage("success", "手机号不正确");
        } else {
          this.getVerify(this.ruleForm.username);
          let t = 60,
            that = this;
          this.disabled = true;
          this.yangName = `60秒后重发`;
          var inter = setInterval(() => {
            t--;
            this.yangName = `${t}秒后重发`;
            if (t <= 0) {
              clearInterval(inter);
              this.yangName = `获取验证码`;
              this.disabled = false;
    ...
    //成功后调用
    gotoPage(data) {
      this.getUserPower(data);
    },
    gotoPage2() {
      this.$store.commit("login", "111");
      this.$store.commit("setAdmin");
    },
    /* 接口 */
    // 获取验证码
    async getVerify(tel) {
      let typeList = await this.Axios.get(`common-api/sms/sendVCode?telephone=${tel}`);
    ...
    //登录
    async login(data) {
      let typeList = await this.Axios.post(`ucenter-api/userlist/login`, data).catch((err) => {
        this.logining = false;
      });
      this.logining = false;
      ...
      if (typeList.data.code == 0) {
        ...
        this.gotoPage(typeList.data.result);
      } else {
        this.showMessage("error", typeList.data.message);
      }
    },
    //获取用户权限
    async getUserPower(userId) {
      this.$store.commit("login", userId);
      this.$store.dispatch("setMenuList", userId).then((res) => {
        this.getUserInfo();
        this.$router.push({ path: this.$store.state.selectPageJurisdiction[0].menus[0].url });
      });
    },
    //获取用户信息
    async getUserInfo() {
      ...
      console.log(typeList.data.result);
      this.addDataStorge("userMessage", typeList.data.result);
    },
    /* 数据处理 */
    //拆分树状图到平行列表
    dataAssembly(data) {
      ...
      function change(data) {
        data.forEach((elem) => {
          if (elem.type == 3) {
            _btnArr.push({ powerId: elem.powerId, name: elem.name, mark: elem.mark, path: elem.path, status: elem.status });
          }
          if (elem.type == 1) {
            _pagArr.push({ powerId: elem.powerId, name: elem.name, mark: elem.mark, path: elem.path, status: elem.status });
          }
          if (elem.type == 0) {
            _sysArr.push({ powerId: elem.powerId, name: elem.name, mark: elem.mark, path: elem.path, status: elem.status });
          }
          if (elem.children != []) {
      ...
      return { sysArr: _newSysArr, pageArr: _newPagArr, btnArr: _newBtnArr };
      //列表展示页数据组装
      //路由数据组装
    },
    //获取组织名称
...
.verify /deep/ .el-form-item__content {
  display: flex;
}
.verify /deep/ .el-form-item__content .el-button--primary {
  margin-left: 5px;
  width: 110px;
  padding: 12px 10px;
}
