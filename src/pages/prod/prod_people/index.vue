 ...
 * @LastEditTime: 2020-12-29 20:11:45
    ...
    <section class="prod_list_content">
      <article class="prod_list_list">
        <div class="prod_attr_button">
          <el-button v-has="'/prod/people-btn-add'" type="primary" size="small" @click="add">+新建专业人员</el-button>
        </div>
        <div class="prod_people_list">
          <el-table :data="tableData" stripe max-height="700px" border style="width: 99%" :header-cell-style="table_heard_style">
            ...
            <el-table-column :formatter="formatIsBType" prop="isB" align="center" label="B本情况" min-width="80"></el-table-column>
            <el-table-column :formatter="formatSocialTyep" prop="socialStatus" align="center" label="社保情况" min-width="80"></el-table-column>
            <el-table-column prop="ageLimit" align="center" label="工作年限" min-width="70"></el-table-column>
            ...
            <el-table-column align="center" label="操作" min-width="80" fixed="right">
              <template slot-scope="scope">
                <el-button v-has="'/prod/people-btn-del'" @click="deleteThis(scope.row)" type="text" size="small">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    async addPeople(data) {
      let _resp = await this.Axios.post(`/product-api/standardlist/add`, data, "postJson");
      if (_resp.data.code == 0) {
        this.showMessage("success", "操作成功");
        this.closeDialog();
        this.getPeople();
    ...
    //查看人才标准
    async getPeople() {
      let _resp = await this.Axios.get(`/product-api/standardlist/page?pageNum=${this.number}&pageSize=${this.pageSize}`);
    ...
    async deletePeople(id) {
      let _resp = await this.Axios.get(`/product-api/standardlist/delete?id=${id}`);
      if (_resp.data.code == 0) {
        this.showMessage("success", "操作成功");
        this.getPeople();
      }
